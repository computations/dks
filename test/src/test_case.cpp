...
void test_attributes(const dks::test_case_t &test_case, bool pt, bool sr,
                     bool rs) {
    ...
    for (auto &&pt : pattern_tips) {
      for (auto &&sr : site_repeats) {
        for (auto &&rs : rate_scalers) {
          SECTION(cpu.first + ", " + pt.first + ", " + sr.first + ", " +
                  rs.first) {
            dks::test_case_t test_case(cpu.second, pt.second, sr.second,
                                       rs.second, 0);
            test_attributes(test_case, pt.second, sr.second, rs.second);
          }
        }
      }
    }
